...
def _append_summary_sections(lines: list[str], summary: str) -> None:
    ...
    if not text:
        lines.append("*未生成 AI 点评。若要启用，请配置 MiniMax API Key。*")
        return
...
def export_markdown(data: dict, filepath: str) -> None:
    ...
    if subtitles:
        ...
        for sub in subtitles:
            lines.append("<details open>")
            lines.append(
                f"<summary><strong>{sub['lang']}（{len(sub['entries'])} 条）</strong></summary>"
            )
            lines.append("")
    ...
    lines.append("<details open>")
    lines.append(
        f"<summary><strong>评论区（{len(comments)} 条一级评论，{total_replies} 条子评论）</strong></summary>"
    )
    lines.append("")
...
def update_summary_in_markdown(filepath: str, summary: str) -> None:
    ...
    placeholder = "*未生成 AI 点评。若要启用，请配置 MiniMax API Key。*"
